...
class PrettyLog {
    ...
    baseLog(title: string, text: string, type: LogType) {
    ...
    _easyLog(type: LogType, textOrTitle: string, content?) {
        const title = content ? textOrTitle : undefined;
        const text = content ?? textOrTitle;
        this.baseLog(title, text, type);
    }

    /**打印普通消息 */
    info(textOrTitle: string, content?) {
        this._easyLog(LogType.info, textOrTitle, content);
    }

    /**打印错误消息 */
    error(textOrTitle: string, content?) {
        this._easyLog(LogType.error, textOrTitle, content);
    }
    /**打印警告消息 */
    warning(textOrTitle: string, content?) {
        this._easyLog(LogType.warn, textOrTitle, content);
    }
    /**打印成功消息 */
    success(textOrTitle: string, content?) {
        this._easyLog(LogType.success, textOrTitle, content);
    }
