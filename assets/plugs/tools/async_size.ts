import { _decorator, ccenum, CCFloat, Component, Layout, Node, UITransform } from 'cc';
...
export class AsyncSize extends Component {
	...
	onSizeChange() {
		//若有layout,则延迟一帧执行,因为layout的size是下一帧才真正变化完全的
		if (this.uiOriginObj?.node.getComponent(Layout)) {
			this.scheduleOnce(() => {
				this._onSizeChange();
			});
		} else {
