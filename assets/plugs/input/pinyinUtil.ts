...
import { pinyin_dict_myword } from './pinyin_dict_myword';
...
class PinYinUtil {
    ...
    parseDict() {
        ...
        if (this.pinyin_dict_notone) {
            this.dict.notone = {};
            this.dict.py2hz = this.pinyin_dict_notone; // 拼音转汉字
            for (let i in this.pinyin_dict_notone) {
                let temp = this.pinyin_dict_notone[i];
        ...
        if (this.pinyin_dict_withtone) {
            this.dict.withtone = {}; // 汉字与拼音映射，多音字用空格分开，类似这种结构：{'大': 'da tai'}
            let temp = this.pinyin_dict_withtone.split(',');
            ...
            if (this.pinyin_dict_notone) {
                ...
                let notone = this.removeTone(this.pinyin_dict_withtone).split(',');
                let py2hz = {},
                    py,
                    hz;
        ...
        if (this.pinyin_dict_myworld) {
            this.dict.py2myword = {};
            this.dict.py2myword = this.pinyin_dict_myworld;
        }
    ...
    getPinyin(chinese, splitter, withtone, polyphone) {
        ...
        let result = [];
        if (this.dict.withtone) {
            // 优先使用带声调的字典文件
            let noChinese = '';
            for (let i = 0, len = chinese.length; i < len; i++) {
                let pinyin = this.dict.withtone[chinese[i]];
                if (pinyin) {
        ...
        if (!polyphone) return result.join(splitter);
            ...
            if (this.pinyin_dict_polyphone) return this.parsePolyphone(chinese, result, splitter, withtone);
            else return this.handlePolyphone(result, ' ', splitter);
        }
    }
    /**
     * 获取汉字的拼音首字母
     ...
     */
    getFirstLetter(str: string, polyphone: boolean) {
        polyphone = polyphone == undefined ? false : polyphone;
        if (!str || /^ +$/g.test(str)) return '';
        if (this.dict.firstletter) {
            // 使用首字母字典文件
            let result = [];
     ...
     */
    getHanzi2(pinyin) {
        let myword = this.getMyWorld(pinyin);

        let result = this.getSingleHanzi(pinyin);
        if (result) return [[...myword, ...result.split('')], pinyin];
        let temp = '';
        for (let i = 0, len = pinyin.length; i < len; i++) {
            ...
            if (!flag) return [[...myword, ...result.split('')], pinyin.substr(0, i + 1) + "'" + pinyin.substr(i + 1)];
        }
    ...
    getMyWorld(pinyin) {
        ...
        for (let i = 0, len = pinyin.length; i < len; i++) {
            temp += pinyin[i];
            result = this.dict.py2myword[temp];
            if (!result) continue;
            let flag = false;
            if (i + 1 < pinyin.length) {
                for (let j = 1, len = pinyin.length; j <= 5 && i + j < len; j++) {
            ...
            if (!flag) return result.split(',');
        }
        //模糊匹配,3个
        let count = 0;
        for (let k in this.dict.py2myword) {
            if (k.includes(pinyin)) {
                count++;
                result = result ? result + ',' + this.dict.py2myword[k] : this.dict.py2myword[k];
                if (count >= 3) return result.split(',');
            }
        }
        if (count > 0) return result.split(',');
        return [];
    }
    ...
    getSameVoiceWord(hz, sameTone) {
        ...
        return this.getHanzi(this.getPinyin(hz, ' ', false, true));
    }
    /**
    ...
    getTone(pinyinWithoutTone) {
        let newToneMap = {};
        for (let i in this.toneMap) newToneMap[this.toneMap[i]] = i;
     ...
     */
    handlePolyphone(array, splitter, joinChar) {
        splitter = splitter || '';
        let result = [''],
            temp = [];
     ...
     */
    parsePolyphone(chinese, result, splitter, withtone) {
        let poly = this.pinyin_dict_polyphone;
        ...
        if (temp) {
            ...
            for (let i = 0; i < temp.length; i++) {
                ...
                if (!withtone) result[i] = this.removeTone(result[i]);
            }
        ...
        for (let i = 0; i < chinese.length; i++) {
            ...
            for (let j = 0; j < max && i + j < chinese.length; j++) {
                ...
                if (res) {
                    ...
                    res = res.split(' ');
                    for (let k = 0; k <= j; k++) {
                        if (res[k]) result[i + k] = withtone ? res[k] : this.removeTone(res[k]);
                    }
                    break;
                }
            }
...
export const pinYinUtil = new PinYinUtil();
pinYinUtil.pinyin_dict_firstletter = pinyin_dict_firstletter;
pinYinUtil.pinyin_dict_notone = pinyin_dict_notone;
pinYinUtil.pinyin_dict_myworld = pinyin_dict_myword;
